...
class Media {
  ...
  update(
    ...
    // Increased margin to prevent early disappearing (clipping)
    // We add a full width buffer to ensure it's fully off-screen before looping
    const margin = this.width * 1.5;
    
    this.isBefore = this.plane.position.x + planeOffset < -viewportOffset - margin;
    this.isAfter = this.plane.position.x - planeOffset > viewportOffset + margin;
  ...
  onResize(
    ...
    if (isMobile) {
        // Mobile optimization:
        // Use a fixed relationship to viewport width to prevent overlap
        // and increase padding for better separation.
        this.plane.scale.x = this.viewport.width * 0.70; // 70% of screen width (reduced from 0.75)
        this.plane.scale.y = this.plane.scale.x * 1.25; // Keep aspect ratio
        this.padding = 2.0; // Reduced padding from 3.0 to 2.0
    } else {
        // Desktop calculation
        this.plane.scale.y =
